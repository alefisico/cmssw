...
#EVENT LOOP
events = Events ('testJetTools.root')
PUMethod = 'CHS'
...
label = ("packedPatJetsSoftDrop")
recursivehandle = Handle("std::vector<pat::Jet>")
recursivelabel = ("packedPatJetsRecursiveSoftDrop")
...
for event in events:
	...
	jets = handle.product()
	event.getByLabel( recursivelabel, recursivehandle )
	recurjets = recursivehandle.product()
	...
	for i,j in enumerate(jets):
		if j.pt() < 200: continue
                print nevents, 'SB', j.pt(), j.eta(), j.userFloat("ak8PFJetsCHSSoftDropMass"), j.userFloat("ak8PFJetsCHSRecursiveSoftDropMass")
                for k in xrange(len(j.subjets("SoftDrop"))):
                    print nevents, 'subjetSB', j.subjets("SoftDrop")[k].pt()
	for i,j in enumerate(recurjets):
		if j.pt() < 200: continue
                print nevents, 'RSB', j.pt(), j.eta(), j.userFloat("ak8PFJetsCHSSoftDropMass"), j.userFloat("ak8PFJetsCHSRecursiveSoftDropMass")
                for k in xrange(len(j.subjets("RecursiveSoftDrop"))):
                    print nevents, 'subjetRSB', j.subjets("RecursiveSoftDrop")[k].pt()
#		print i, j.bDiscriminator("pfDeepCSVJetTags:probb")
#		print i, 'probb', j.bDiscriminator("pfDeepFlavourJetTags:probb")
#		print i, 'probbb', j.bDiscriminator("pfDeepFlavourJetTags:probbb")
#		print i, 'problepb', j.bDiscriminator("pfDeepFlavourJetTags:problepb")
#		print i, 'probc', j.bDiscriminator("pfDeepFlavourJetTags:probc")
#		print i, 'probuds', j.bDiscriminator("pfDeepFlavourJetTags:probuds")
#		print i, 'probg', j.bDiscriminator("pfDeepFlavourJetTags:probg")
